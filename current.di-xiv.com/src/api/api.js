export default {
  async fetch(request, env) {
    ...
    if (pathname === "/gallery") {
      ...
      return createResponse("Di-XIV", "text/plain");
    }
  },
};
...
async function handleTagsRequest(env, searchParams) {
  try {
    ...
    const tagName = searchParams.get("tag_name"); // Add this line
    ...
    if (tagName) {
      conditions.push("LOWER(t.tag_name) LIKE ?");
      params.push(`%${tagName.toLowerCase()}%`); // Use LIKE for partial matches
    }
...
async function handleGalleriesWithArtworksRequest(env, searchParams) {
  try {
    ...
    const includeArtworkRatingIds = getMultipleQueryParams(
      searchParams,
      "include_artwork_rating_id",
    );
    const includeTagNames = getMultipleQueryParams(
      searchParams,
      "include_tag_name",
    );
    const includeArtistNames = getMultipleQueryParams(
      searchParams,
      "include_artist_name",
    );
    const includeGalleryNames = getMultipleQueryParams(
      searchParams,
      "include_gallery_name",
    );
    let query = `
    ...
    if (includeArtworkRatingIds.length > 0) {
      conditions.push(`
        a.artwork_id IN (
          SELECT ar.artwork_id
          FROM Artwork_Rating ar
          WHERE ar.rating_id IN (${includeArtworkRatingIds.map(() => "?").join(",")})
          GROUP BY ar.artwork_id
          HAVING COUNT(DISTINCT ar.rating_id) = ${includeArtworkRatingIds.length}
        )
      `);
      params.push(...includeArtworkRatingIds);
    }
    ...
    EXISTS (
      ...
      WHERE at.artwork_id = a.artwork_id
      AND (${includeTagNames
        .map(() => `LOWER(t.tag_name) LIKE ?`)
        .join(" OR ")})
    )
  `);
      params.push(...includeTagNames.map((name) => `%${name.toLowerCase()}%`));
    }

    if (includeArtistNames.length > 0) {
      searchConditions.push(`
    EXISTS (
      SELECT 1
      FROM Artist art
      WHERE art.artist_id = a.artist_id
      AND (${includeArtistNames
        .map(() => `LOWER(art.artist_name) LIKE ?`)
        .join(" OR ")})
    )
  `);
      params.push(
        ...includeArtistNames.map((name) => `%${name.toLowerCase()}%`),
      );
    }
    ...
    if (includeGalleryNames.length > 0) {
      searchConditions.push(`
    (${includeGalleryNames
      .map(() => `LOWER(g.gallery_name) LIKE ?`)
      .join(" OR ")})
  `);
      params.push(
        ...includeGalleryNames.map((name) => `%${name.toLowerCase()}%`),
      );
    }

    // Combine search conditions with OR instead of AND
    ...
    if (
      ...
      includeArtistNames.length > 0 ||
      includeGalleryNames.length > 0
    ) {
    ...
    console.log("Search parameters:", {
      tagNames: includeTagNames,
      artistNames: includeArtistNames,
      galleryNames: includeGalleryNames,
    });
    ...
    const groupedResults = results.reduce((acc, row) => {
      ...
      if (artwork) {
        ...
        if (
          artwork_rating_id &&
          !artwork.ratings.some(
            (rating) => rating.rating_id === artwork_rating_id,
          )
        ) {
          artwork.ratings.push({
            rating_id: artwork_rating_id,
            rating_name: artwork_rating_name,
          });
        }
      } else {
    ...
    // Filter out galleries with no artworks
    const filteredResults = Object.values(groupedResults).filter(
      (gallery) => gallery.artworks.length > 0,
    );

    return createResponse(JSON.stringify(filteredResults));
  } catch (error) {
    console.error("Error:", error);
    return createResponse(
      JSON.stringify({ error: "Database query failed" }),
