...
const CopyButton: React.FC<{ text: string; locale: string }> = ({
  ...
  return (
    <button
      onClick={handleCopy}
      className="ml-2 p-1 bg-gray-200 hover:bg-gray-300 rounded flex-shrink-0"
      title={
        copied
          ...
          <div key={index} className="flex flex-col">
            ...
            <div className="text-sm space-y-1">
              <div className="flex justify-between items-center">
                <span className="regular">HEX:</span> <span>#{color.hex}</span>
                <CopyButton text={`#${color.hex}`} locale={safeLocale} />
              </div>
              <div className="flex justify-between items-center">
                <span className="regular">RGB:</span> <span>{color.rgb}</span>
                <CopyButton text={color.rgb} locale={safeLocale} />
              </div>
              <div className="flex justify-between items-center">
                <span className="regular">HSL:</span> <span>{color.hsl}</span>
