import React, { useState, useEffect } from "react";

interface Artist {
  artist_name: string;
  website: string | null;
}

interface Artwork {
  artwork_id: number;
  artwork_url: string;
  artist: Artist;
}

interface Gallery {
  gallery_id: number;
  gallery_name: string;
  artworks: Artwork[];
}

const LastGallery: React.FC = () => {
  const [gallery, setGallery] = useState<Gallery | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchLastGallery = async () => {
      try {
        setLoading(true);
        const response = await fetch("https://api.di-xiv.com/gallery");

        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }

        const data: Gallery[] = await response.json();

        if (data.length > 0) {
          // Assuming the API returns galleries sorted by ID, get the last one
          const lastGallery = data[data.length - 1];
          setGallery(lastGallery);
        } else {
          setError("No galleries found");
        }
      } catch (err) {
        setError("Failed to fetch gallery data");
        console.error(err);
      } finally {
        setLoading(false);
      }
    };

    fetchLastGallery();
  }, []);

  if (loading) return <div>Loading...</div>;
  if (error) return <div>Error: {error}</div>;
  if (!gallery) return <div>No gallery found</div>;

  return (
    <div>
      {gallery.artworks.map((artwork) => (
        <div>
          <div className="mb-[5%] mt-[5%] glassbox rounded p-1">
            <div className="flex justify-evenly items-center select-none">
              <div className="flex items-center" title={gallery.gallery_name}>
                <div className="dark:invert items-center inline-block mr-2">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="dark:invert inline-block h-8 mobile-only:h-5 w-auto"
                    viewBox="0 0 16 16"
                  >
                    <path
                      fill="currentColor"
                      d="M1.75 3a.75.75 0 0 0 0 1.5h12.5a.75.75 0 0 0 0-1.5zm0 3a.75.75 0 0 0 0 1.5h12.5a.75.75 0 0 0 0-1.5zM1 9.75A.75.75 0 0 1 1.75 9h12.5a.75.75 0 0 1 0 1.5H1.75A.75.75 0 0 1 1 9.75M1.75 12a.75.75 0 0 0 0 1.5h8.5a.75.75 0 0 0 0-1.5z"
                    />
                  </svg>
                </div>
                <p className="regular mobile-only:text-sm inline-block">
                  {gallery.gallery_name}
                </p>
              </div>
              <div key={artwork.artwork_id} className="flex items-center">
                <div className="dark:invert items-center inline-block mr-2">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="dark:invert inline-block h-6 mobile-only:h-5 w-auto"
                    viewBox="0 0 16 16"
                  >
                    <path
                      fill="currentColor"
                      d="M11.134 1.535c.7-.509 1.416-.942 2.076-1.155c.649-.21 1.463-.267 2.069.34c.603.601.568 1.411.368 2.07c-.202.668-.624 1.39-1.125 2.096c-1.011 1.424-2.496 2.987-3.775 4.249c-1.098 1.084-2.132 1.839-3.04 2.3a3.74 3.74 0 0 1-1.055 3.217c-.431.431-1.065.691-1.657.861c-.614.177-1.294.287-1.914.357A21 21 0 0 1 .797 16H.743l.007-.75H.749L.742 16a.75.75 0 0 1-.743-.742l.743-.008l-.742.007v-.054a21 21 0 0 1 .13-2.284q.101-.972.358-1.914c.17-.591.43-1.226.86-1.657a3.75 3.75 0 0 1 3.227-1.054c.466-.893 1.225-1.907 2.314-2.982c1.271-1.255 2.833-2.75 4.245-3.777M1.62 13.089q-.077.696-.104 1.395q.7-.027 1.396-.104a10.5 10.5 0 0 0 1.668-.309c.526-.151.856-.325 1.011-.48a2.25 2.25 0 1 0-3.182-3.182c-.155.155-.329.485-.48 1.01a10.5 10.5 0 0 0-.309 1.67m10.396-10.34c-1.224.89-2.605 2.189-3.822 3.384l1.718 1.718c1.21-1.205 2.51-2.597 3.387-3.833c.47-.662.78-1.227.912-1.662c.134-.444.032-.551.009-.575h-.001V1.78c-.014-.014-.113-.113-.548.027c-.432.14-.995.462-1.655.942m-4.832 7.266a10 10 0 0 0 1.63-1.142L7.155 7.216a9.7 9.7 0 0 0-1.161 1.607c.482.302.889.71 1.19 1.192"
                    />
                  </svg>
                </div>
                {artwork.artist.website && (
                  <a
                    href={artwork.artist.website}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="regular mobile-only:text-sm"
                    title={`${artwork.artist.artist_name}: ${artwork.artist.website}`}
                  >
                    {artwork.artist.artist_name}↗
                  </a>
                )}
              </div>
            </div>
          </div>
          <img
            src={artwork.artwork_url}
            alt={`Artwork ${artwork.artwork_id}`}
            className="rounded"
          />
        </div>
      ))}
    </div>
  );
};

export default LastGallery;
