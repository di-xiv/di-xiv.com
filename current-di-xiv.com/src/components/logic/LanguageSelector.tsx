...
const LanguageSelector: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const isOnCorrectLangPage = supportedLangs.some((supportedLang) =>
    ...
    if (!isOnCorrectLangPage) {
      ...
      if (initialLang !== defaultLang) {
        newPath = `/${initialLang}${currentPath}`;
      } else if (
        currentPath !== "/" &&
        ...
        newPath = currentPath.replace(/^\/(en|es|ja|pt)/, "");
      }
      if (newPath) {
  ...
  return (
    <div
      ...
      className="flex gap-2 justify-end items-center select-none mb-2"
    >
      ...
      <div id="selector-wrapper">
        <select
          ...
          aria-label="language-selector"
          className="grid grid-cols-1 light p-2 mobile-only:p-1 glassbox"
          value={lang}
          onChange={handleLanguageChange}
        >
          <option value="en" className="light bg-[#181a1b] text-[#efefef]">
            🇬🇧 EN
          </option>
          <option value="es" className="light bg-[#181a1b] text-[#efefef]">
            🇪🇸 ES
          </option>
          <option value="pt" className="light bg-[#181a1b] text-[#efefef]">
            🇧🇷 PT
          </option>
          <option value="ja" className="light bg-[#181a1b] text-[#efefef]">
            🇯🇵 JA
          </option>
        </select>
      </div>
