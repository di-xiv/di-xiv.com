...
const LanguageSelector: React.FC = () => {
  ...
  const supportedLangs = ["en", "es", "ja", "pt"];
  ...
  useEffect(() => {
    ...
    if (supportedLangs.includes(userLang.split("-")[0])) {
      initialLang = userLang.split("-")[0];
    }
    ...
    const urlLang = initialLang === "ja" ? "jp" : initialLang;
    const isOnCorrectLangPage =
      supportedLangs.some((supportedLang) =>
        currentPath.startsWith(`/${supportedLang}`),
      ) || currentPath.startsWith("/jp");

    if (!isOnCorrectLangPage) {
      let newPath;
      if (initialLang !== defaultLang) {
        newPath = `/${urlLang}${currentPath}`;
      } else if (
        currentPath !== "/" &&
        (supportedLangs.some((supportedLang) =>
          currentPath.startsWith(`/${supportedLang}`),
        ...
        newPath = currentPath.replace(/^\/(en|es|ja|jp|pt)/, "");
      }
      if (newPath) {
        console.log("Redirecting to:", newPath + queryString);
  ...
  const handleLanguageChange = (
    ...
    const selectedUrlLang = selectedLang === "ja" ? "jp" : selectedLang;
    const currentPath = window.location.pathname;
    const queryString = window.location.search;
    const pathSegments = currentPath.split("/");

    let newPath;
    if (supportedLangs.includes(pathSegments[1]) || pathSegments[1] === "jp") {
      if (selectedLang === defaultLang) {
        pathSegments.splice(1, 1);
        newPath = pathSegments.join("/") || "/";
      } else {
        pathSegments[1] = selectedUrlLang;
        newPath = pathSegments.join("/");
      }
    } else {
      newPath = `/${selectedUrlLang}${currentPath}`;
    }

    if (newPath !== currentPath) {
      console.log("Redirecting to (on change):", newPath + queryString);
      window.location.href = newPath + queryString;
    }
  };
  ...
  return (
    ...
    >
      <label htmlFor="language-selector" aria-label="language-selector">
        <div id="translate-icon" aria-label="language-selector">
          <svg
      ...
      <div id="selector-wrapper">
        <select
          ...
          aria-label="language-selector"
          className="grid grid-cols-1 ultralight p-2 mobile-only:p-1 glassbox"
          value={lang}
          onChange={handleLanguageChange}
        >
          <option value="en" className="ultralight bg-[#181a1b] text-[#efefef]">
            {" "}
            🇬🇧 EN{" "}
          </option>
          <option value="es" className="ultralight bg-[#181a1b] text-[#efefef]">
            {" "}
            🇪🇸 ES{" "}
          </option>
          <option value="pt" className="ultralight bg-[#181a1b] text-[#efefef]">
            {" "}
            🇧🇷 PT{" "}
          </option>
          <option value="jp" className="ultralight bg-[#181a1b] text-[#efefef]">
            {" "}
            🇯🇵 JP{" "}
          </option>
        </select>
